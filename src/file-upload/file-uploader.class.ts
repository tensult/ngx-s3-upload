...
export class FileUploader {
  ...
  protected _s3Upload(item:FileItem):any {
    ...
    let s3Params = {
      ...
      Key: this.options.s3KeyPrefix + "/" + item._file.name
    }
    let s3Upload = item._s3Upload = s3.upload(s3Params);
    s3Upload.on("httpUploadProgress", (event: AWS.S3.ManagedUpload.Progress) => {
